...
void __no_inline_not_in_flash_func(SDEraseBlock)(uint32_t flash_offs, size_t count) {
    ...
    rom_SDEraseBlock_fn SDEraseBlock_func = (rom_SDEraseBlock_fn)rom_func_lookup_inline(ROM_FUNC_SDEraseBlock);
    ...
    assert(connect_internal_flash_func && flash_exit_xip_func && SDEraseBlock_func && flash_flush_cache_func);
    ...
    flash_exit_xip_func();
    SDEraseBlock_func(flash_offs, count, FLASH_BLOCK_SIZE, FLASH_BLOCK_ERASE_CMD);
...
void __no_inline_not_in_flash_func(SDWriteProg)(uint32_t flash_offs, const uint8_t *data, size_t count) {
#ifdef PICO_FLASH_SIZE_BYTES
    hard_assert(flash_offs + count <= PICO_FLASH_SIZE_BYTES);
#endif
    ...
    rom_connect_internal_flash_fn connect_internal_flash_func = (rom_connect_internal_flash_fn)rom_func_lookup_inline(ROM_FUNC_CONNECT_INTERNAL_FLASH);
    rom_flash_exit_xip_fn flash_exit_xip_func = (rom_flash_exit_xip_fn)rom_func_lookup_inline(ROM_FUNC_FLASH_EXIT_XIP);
    rom_SDWriteProg_fn SDWriteProg_func = (rom_SDWriteProg_fn)rom_func_lookup_inline(ROM_FUNC_SDWriteProg);
    rom_flash_flush_cache_fn flash_flush_cache_func = (rom_flash_flush_cache_fn)rom_func_lookup_inline(ROM_FUNC_FLASH_FLUSH_CACHE);
    assert(connect_internal_flash_func && flash_exit_xip_func && SDWriteProg_func && flash_flush_cache_func);
    flash_init_boot2_copyout();
    xip_cache_clean_all();
#if PICO_RP2350
    flash_rp2350_qmi_save_state_t qmi_save;
    ...
    connect_internal_flash_func();
    flash_exit_xip_func();
    SDWriteProg_func(flash_offs, data, count);
    flash_flush_cache_func(); // Note this is needed to remove CSn IO force as well as cache flushing
    flash_enable_xip_via_boot2();
#if PICO_RP2350
    flash_rp2350_restore_qmi_cs1(&qmi_save);
